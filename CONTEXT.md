...
- **手动旋转 (Manual Rotation)**：预览期用户在全屏图片查看器触发的像素级顺时针 90° 旋转落盘（2026-09-20 决议，ADR-0029）——复用「判向自动旋转」的旋转与收尾管线（同格式重编码 → 清视觉队列残留 → FileWatcher 重建重转写），但入口相反：用户显式点击、无需视觉判向、**无条件可用**（不跟随 `VISION_AUTO_ROTATE` 开关与视觉 API 配置）、不弹确认（连点累加角度）。作用范围 = 经 /api/preview/raw 服务的知识库图像文件（jpg/jpeg/png/webp，含 md 相对路径引用图与日记照片）；文档内嵌图片（ImageStore 提取产物）不提供。与图像预览 writable=False（文本写回闸门）并存不冲突——旋转是独立 mutation 通道。_Avoid_: 显示态 CSS 旋转（刷新即失效，治标不治本）、EXIF Orientation 标记（继承判向自动旋转的否决）。
