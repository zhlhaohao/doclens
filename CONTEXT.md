...
- 2026-09-17：LLM 追踪落盘（无 ADR，输出层可逆）= planify 框架层 `LLMTracer`（provider 四入口可选 tracer 参数，逐调用传入）；实发请求体 + 实收原生响应（Anthropic final_message dump / OpenAI 流聚合还原）双节 md；聊天会话键追加同文件；命中率不落盘，会话信息弹窗新增全会话累计命中率行（SSE usage 逐条累加 + items 聚合恢复，extractLastUsage 升级 aggregateUsage）；GitSync 自动 gitignore 扩展 `.planify/`（llm_trace 含对话全文，防进知识库同步）。
