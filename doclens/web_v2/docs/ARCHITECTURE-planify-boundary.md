...
| 层 | 模块 | 职责 | 官方扩展点 |
|---|---|---|---|
| 装配层 | `bootstrap.py` / `core/session_manager.py` | Session 组件装配、Provider 单例、配置注册 | `register_app_dependencies()` / `register_planify_config()` / `get_or_create_session()` |
...
### 🔴 严重：装配层整体绕过，手工复刻 `initialize_session_components()`

`doclens/agent_integration.py:132-328`（`CortexAgent.initialize`）**完全绕过** planify 的装配入口（`bootstrap.get_or_create_session()` / `SessionManager.initialize_session_components()`），手工复刻全部装配逻辑——对照两边代码几乎逐行同构：

| planify 官方路径（`session_manager.py:277-346`） | doclens 复刻（`agent_integration.py`） |
|---|---|
| `session.logger = setup_logging(...)` | `:179-184` 同样调 `setup_logging` |
...
| `session.client = provider` 等属性赋值 | `:315-325` 直接对 `Session` 属性逐个赋值 |

后果：

- **双写维护**：planify 装配逻辑任何演进（新 Manager、新参数）都必须手工同步到 doclens，否则静默缺失。与 planify 技术债 #1（双 runner）同构，是「双装配器」债。
- **两套装配并存**：`doclens/web_v2/deps.py:158` 的 `reload_config()` 还去调 `SessionManager.invalidate_provider()`——但 Web 聊天路径用的是 `CortexAgent` 手工装配的 session，**SessionManager 那条装配线从未产生实际会话**，invalidate 的是一个没有消费者的缓存。代码读起来像在用官方路径，实际不是。
- 附带 hack：`agent_integration.py:18` 的 `sys.path.insert(0, ...)` 运行时改模块搜索路径。

### 🟠 中：宿主每请求改写 planify 共享 Session 内部状态

`doclens/web_v2/api/chat.py:110-113`：每个 chat 请求都对**全局单例 session** 的 `tool_handlers` 字典执行 `bind_user_interaction_handlers` 重绑。绑定机制本身是官方扩展点，但「往共享单例上 per-request 重绑」越过其设计意图——隐含「单会话单流」假设，同 session 并发两流会互相覆盖绑定（ARCHITECTURE-sse.md 残留风险 #3）。

同理 `apply_config()`（`agent_integration.py:354-360`）直接写 `session.config.model_id` 等字段，绕过配置注册通路（`register_planify_config`），注释自述是为了绕过只读 property——说明 planify 没有提供热更新接口，宿主被迫戳内部。
...
- **会话持久化由 doclens 自建**（`sessions_store.py`）：planify 文档承认持久化缺位（技术债 #5），宿主补位合理；但意味着 planify 的 `Session.messages` 在 Web 路径被架空，历史真相在 doclens 的 SQLite。
...
| doclens → planify | per-request 改写共享 session 的 tool_handlers；绕过配置通路直写 SessionConfig | 🟠 |
| planify → doclens | prompts.py 硬编码 KB 业务策略；logging_config 反向定位 doclens 包做首启初始化 | 🔴 |
| 双向 | 配置双通路（doclens env 直读 vs `register_planify_config`）+ SessionManager 形同虚设 | 🟠 |
...
3. `reload_provider()` 公共 API 替代 `apply_config()` 直写 SessionConfig；
4. 首启工作区初始化（拷 skills/.env.example）从 `logging_config.py` 移到 doclens 启动流程；
5. 热重载后删除 `deps.py` 里对 `SessionManager.invalidate_provider()` 的空调用，或让装配真正走 SessionManager 二选一。
...
| 2 | 装配层双写 | **最小变体（刻意不做完全收敛）**：SessionManager 官方装配缺 gui_mode / skill_access_state / 自定义 skills_dir 等 5+ 插槽，收敛风险大于收益。改为：删除 `deps.py` 的 `SessionManager.invalidate_provider()` 死调用（web 路径无消费者）与 import；`CortexAgent.initialize` docstring 显式声明装配归属——静默技术债转为文档化决策 | `doclens/web_v2/deps.py`、`doclens/agent_integration.py` |
| 3 | apply_config 直写 SessionConfig | planify 新增 `Session.update_llm_config()` 官方热更新 API（重建 Provider + 更新 config 字段，None 表示不动）；`CortexAgent.apply_config` 改为薄适配层。顺手删除 `session.py` 死导入 `from anthropic import Anthropic`（planify 技术债 #3 一半） | `planify/core/session.py`、`doclens/agent_integration.py` |
...
| 5 | per-request 改写共享 tool_handlers | `chat.py` 与 `agent_integration.run_query` CLI 分支改为 `{**session.tool_handlers}` 浅拷贝后 bind——顺带修复同 session 并发两流 emitter 交叉接线的真实 bug | `doclens/web_v2/api/chat.py`、`doclens/agent_integration.py` |
