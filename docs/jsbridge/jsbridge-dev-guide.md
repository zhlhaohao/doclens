...
| `downloadFile` | JsDownloadFile | 异步 | `{downloadUrl(必填), fileName="", cookieName="cortex_auth"}` | success `{code,name,savedTo,bytes,notified}`（无 cancel 态）；**Android 侧待实现**（契约已冻结），详见 `docs/download_bridge.md` |
