# ADR-0010: 搜索预设——复用模型预设体系（kind 扩展）

搜索调优参数（3 过滤 + 5 评分权重）做成命名档案一键切换，**复用** ADR-0009 的模型预设整套机制（同一 `model_presets.json` / `presets_store` / `/api/presets` / 物化写 global `.env` / 激活键），以 `kind: "search"` 扩展，而非独立一套 `search_presets`。
