...
- `types.py`：`EventEmitter` 协议（`@runtime_checkable`）+ 8 种事件类型（TEXT / TOOL_CALL / TOOL_RESULT / ASK_USER / DONE / ERROR / HEARTBEAT / USAGE）+ `StreamingConfig` + `ToolCallState`（累积 `input_json_delta` 片段，解析失败返回 `{"_parse_error": raw}` 而非 `{}`）。USAGE（2026-09-17）由 runner 在每次 LLM 调用收尾时 `emit_usage()`（归一化四字段），宿主用于展示上下文占用；注意：**协议加便捷方法对鸭子类型 emitter 是破坏性扩展**——只有显式继承 `EventEmitter` 的实现才自动获得默认实现
